...
from db.user_details import update_user_image_url,update_group_image_url , profile_picture, check_friendship_and_get_profile_pic
    ...
    def post(self):
        ...
        if file:
            ...
            try:
                ...
                image_url = f"https://{self.s3_bucket}.s3.amazonaws.com/{file_name}"
                response = update_group_image_url(username, image_url)
                return make_response(jsonify({"url": image_url, "db": response}), 200)
            except NoCredentialsError as e:
                return make_response(jsonify({"error": "Credentials not available"}), 403)
            except Exception as e:
